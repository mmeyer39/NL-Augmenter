...
def separate_into_contracted_and_expanded_form(abbreviations: List) -> Tuple[List, List]:
    """
...
def create_tokens(text: str) -> List:
    ...
    return [
        {
            "text": element.text,
            "is_a_verb": element.tag_ is not None and element.tag_.startswith("V"),
...
def indexes_of_abbreviations(tokens: List, abbreviation: str) -> Tuple[int, int]:
    """
    Look for indexes of begining and ending of detected abbreviation in input tokens.
...
def merge(tokens: List, start: int, finish: int) -> List:
    ...
    text = "".join([token["text"] + token["end_space"] for token in tokens[start: finish + 1]])
...
class ReplaceAbbreviations(SentenceOperation):
    ...
    def __init__(
        ...
        self.contracted_abbreviations, self.expanded_abbreviations = separate_into_contracted_and_expanded_form(
            self.abbreviations
        )
    ...
    def tag_tokens(self, tokens: List) -> List:
        ...
        while counter < len(tokens):
            ...
            candidate_abbreviations = [abbr
                                       for abbr in self.expanded_abbreviations
                                       if abbr.startswith(token["text"].lower() + token["end_space"])]
            for canditate_abbreviation in candidate_abbreviations:
                # search for the candidate abbreviation in teh rest of the token list.
                start, finish = indexes_of_abbreviations(tokens, canditate_abbreviation)
